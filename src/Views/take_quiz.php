    ...
    <form action="index.php?action=submit_quiz" method="POST">
        ...
        <?php foreach ($questions as $q): ?>
            <div class="mb-8 p-4 border-l-4 border-indigo-500 bg-gray-50">
                <p class="font-semibold text-lg mb-4"><?= htmlspecialchars($q->question) ?></p>
                
                <div class="space-y-2">
                    <label class="flex items-center space-x-3 p-2 hover:bg-indigo-100 rounded cursor-pointer">
                        <input type="radio" name="answer[<?= $q->q_id ?>]" value="<?= $q->a_id ?>" class="form-radio h-5 w-5 text-indigo-600" required>
                        <span><?= htmlspecialchars($q->answer_text) ?></span>
                    </label>
                </div>
            </div>
        <?php endforeach; ?>
